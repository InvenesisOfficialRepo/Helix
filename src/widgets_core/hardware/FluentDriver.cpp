...
bool FluentDriver::generate(const QJsonObject &exp,
  ...
  const bool is384 = (exp.value("plate_type").toString() == "384");
      ...
      cmpBestPlan; // compound_name -> {testId, maxStartConc}
  {
    ...
    for (const auto &rv : allReqs) {
      ...
      const QString cn = ro.value("compound_name").toString().trimmed().toLower();
      const QString tid = ro.value("requested_tests").toString().trimmed();
      double sc = convertToMicroM(
          readDouble(ro, "starting_concentration", 0.0),
          ro.value("starting_concentration_unit").toString()
      );
  ...
  double globalStartConcMicroM = 0.0;
  {
    ...
    if (!allReqs.isEmpty()) {
      ...
      globalStartConcMicroM = convertToMicroM(
          readDouble(tr0, "starting_concentration", 0.0),
  ...
  if (!hasExplicitStd && !stdName.isEmpty() && !availableStandards.isEmpty()) {
    ...
    if (!testRequests.isEmpty()) {
      ...
      double startConc = convertToMicroM(
          readDouble(tr0, "starting_concentration", 100.0),
          tr0.value("starting_concentration_unit").toString()
      );
  ...
  double stockMicroM = convertToMicroM(
      readDouble(exp.value("test_requests").toArray().first().toObject(), "stock_concentration", 10000.0),
      exp.value("test_requests").toArray().first().toObject().value("stock_concentration_unit").toString()
  );
  ...
  auto computePlanFor = [&](const QJsonObject &co) -> PerCmpPlan {
    ...
    // per-compound stock (normalize to uM using convertToMicroM)
    double cStock = convertToMicroM(
        readDouble(co, "concentration", p.stockConc),
        co.value("concentration_unit").toString()
    );
    ...
    // test_request row.
    const QString cName = co.value("product_name").toString().trimmed().toLower();
  ...
  auto produceDaughterPlateMap =
    ...
    for (const QString &start : sortedStd) {
      ...
      for (int i = 0; i < chain.size(); ++i) {
        ...
        r.volumeUnit = "ul";
        r.conc = concs.value(toA01(chain.at(i))).toDouble();
        r.concUnit = "uM";
        r.u1 = QString("%1_%2").arg(r.containerBarcode, r.wellA01);
    ...
    for (const auto &p : placedStart) {
      ...
      r0.volumeUnit = "ul";
      r0.conc = concs.value(toA01(dst)).toDouble();
      r0.concUnit = "uM";
      ...
      for (int s = 1; s < nDilC; ++s) {
        ...
        rd.volumeUnit = "ul";
        rd.conc = concs.value(toA01(wn)).toDouble();
        rd.concUnit = "uM";
    ...
    QList<DaughterPlateEntry> allRows = fillMissingWells(rows, dghtBarcode, is384, projectCode, QString::number(nDilGlob));

    FileOut fcsv;
    fcsv.relativePath =
        QString("PlateMapHitLW/%1.csv").arg(QString("Daughter_%1").arg(di + 1));
    fcsv.lines = renderPlateMapCSV(allRows);
    fcsv.isAux = true;
    return fcsv;
  };
  ...
  for (int di = 0; di < plates.size(); ++di) {
    ...
    for (const auto &h : hitsSorted) {
      ...
      if (step != 0) {
        int cur = startIdx;
        const int nDilC = std::max(1, perPlan.value(h.product, makeDefaultPlan()).nDil);
        for (int s = 1; s < nDilC; ++s) {
          const int nxt = cur + step;
          if (nxt < 1 || nxt > 96)
            break;
  ...
  for (int ti = 0; ti < testPlates.size(); ++ti) {
    ...
    for (auto it = wells.begin(); it != wells.end(); ++it) {
      ...
      const bool isStdWell = baseCmp.startsWith("Standard", Qt::CaseInsensitive) ||
                             cmpName.contains("Standard", Qt::CaseInsensitive);
      ...
      if (baseCmp.compare("DMSO", Qt::CaseInsensitive) == 0) {
        ...
        VolumePlanEntry tempVpe;
        QString tempErr;
        double cStock = stockMicroM;
        if (isStdWell) {
          cStock = stdConc;
        } else if (cmpIndex.contains(baseCmp)) {
          cStock = perPlan.value(baseCmp).stockConc;
        }
      ...
      } else if (isStdWell && useMatrixStandard) {
  ...
  // ---- Export single QC plate and DMSO plate layouts (96-well only) ----
  if (!is384) {
    const QJsonArray qcPlates = exp.value("qc_plates").toArray();
    if (!qcPlates.isEmpty()) {
      const QJsonObject plate = qcPlates.at(0).toObject();
      const QString qcBarcode = "QC";
      const QString projectCode = exp.value("project_code").toString();
      const QJsonObject wells = plate.value("wells").toObject();
      const QJsonObject concs = plate.value("concentrations").toObject();
      ...
      for (auto it = wells.begin(); it != wells.end(); ++it) {
        ...
        const bool isStdWell = (baseCmp.compare("DMSO", Qt::CaseInsensitive) != 0);

        DaughterPlateEntry r;
        r.containerBarcode = qcBarcode;
        r.sampleAlias = cmpName;
        r.wellA01 = toA01(wn);

        double volFinal = vpe.volFinal > 0.0 ? vpe.volFinal : 100.0;
      ...
      QString dilStepsStr = QString::number(
          readDouble(exp.value("test_requests").toArray().at(0).toObject(),
                     "dilution_steps", 0.0));
      QList<DaughterPlateEntry> allRows = fillMissingWells(rows, qcBarcode, is384, projectCode, dilStepsStr);

      FileOut fcsv;
      fcsv.relativePath = QString("PlateMapHitLW/QC.csv");
      fcsv.lines = renderPlateMapCSV(allRows);
      fcsv.isAux = true;
      outs.push_back(std::move(fcsv));
    }

    // DMSO control plate (96 wells, all DMSO)
    {
      const QString projectCode = exp.value("project_code").toString();
      QString dilStepsStr = QString::number(
          readDouble(exp.value("test_requests").toArray().at(0).toObject(),
                     "dilution_steps", 0.0));
      QList<DaughterPlateEntry> dmsoRows;
      QStringList rowLetters = {"A", "B", "C", "D", "E", "F", "G", "H"};
      double volFinal = vpe.volFinal > 0.0 ? vpe.volFinal : 100.0;
      for (const QString &r : rowLetters) {
        for (int c = 1; c <= 12; ++c) {
          const QString well = r + QString::number(c).rightJustified(2, '0');
          DaughterPlateEntry rd;
          rd.containerBarcode = "DMSO";
          rd.sampleAlias = "DMSO";
          rd.wellA01 = well;
          rd.volumeUL = volFinal;
          rd.volumeUnit = "ul";
          rd.conc = 100.0;
          rd.concUnit = "%";
          rd.u1 = QString("DMSO_%1").arg(well);
          rd.u2 = "DMSO";
          rd.u3 = "100";
          rd.u4 = dilStepsStr;
          rd.u5 = projectCode;
          dmsoRows.push_back(rd);
        }
      }
      FileOut fdmso;
      fdmso.relativePath = QString("PlateMapHitLW/DMSO.csv");
      fdmso.lines = renderPlateMapCSV(dmsoRows);
      fdmso.isAux = true;
      outs.push_back(std::move(fdmso));
    }
  }
  ...
  // ---- Export step-by-step narrative summary ----
  {
    QStringList summaryLines;
    summaryLines << "======================================================================";
    summaryLines << "               INVENESIS EXPERIMENT DILUTION STEPS SUMMARY            ";
    summaryLines << "======================================================================";
    summaryLines << QString("Experiment Code: %1").arg(exp.value("experiment_code").toString());
    summaryLines << QString("User:            %1").arg(exp.value("user").toString());
    summaryLines << QString("Project Code:    %1").arg(exp.value("project_code").toString());
    summaryLines << QString("Date Generated:  %1").arg(QDateTime::currentDateTime().toString(Qt::ISODate));
    summaryLines << "";
    ...
    for (int di = 0; di < plates.size(); ++di) {
      ...
      summaryLines << "======================================================================";
      summaryLines << QString(" DAUGHTER PLATE: %1").arg(dghtBarcodeStr);
      summaryLines << "======================================================================";
      summaryLines << "";

      // Group seed and dilution audit rows for this daughter plate
      QMap<QString, SeedAuditRow> cmpSeeds;
      for (const auto &ar : seedAudit) {
        if (ar.daughterBarcode == dghtBarcodeStr) {
          cmpSeeds.insert(ar.analyte, ar);
        }
      }

      QMap<QString, QList<DilutionAuditRow>> cmpDilutions;
      for (const auto &dr : dilutionAudit) {
        if (dr.daughterBarcode == dghtBarcodeStr) {
          cmpDilutions[dr.analyte].append(dr);
        }
      }

      // 1. Process Standard (if present in seeds)
      QString standardKey = stdName.isEmpty() ? "Standard" : stdName;
      if (cmpSeeds.contains(standardKey)) {
        const auto &seed = cmpSeeds.value(standardKey);
        summaryLines << QString("Standard: %1").arg(standardKey);
        summaryLines << "--------------------------------------------------";
        summaryLines << QString("  1. Preparation of Starting Well (%1):").arg(toA01(seed.startWell));
        summaryLines << QString("     - Took %1 uL of Standard at %2 uM from standard source (barcode %3) at well %4.")
                         .arg(QString::number(seed.seedVolumeUL, 'f', 1))
                         .arg(QString::number(stdConc, 'f', 1))
                         .arg(seed.matrixBarcode)
                         .arg(toA01(seed.matrixWell));
        summaryLines << QString("     - Placed in starting well %1 that contained %2 uL of DMSO.")
                         .arg(toA01(seed.startWell))
                         .arg(QString::number(stdDmsoStart, 'f', 1));
        summaryLines << QString("     - Resulting starting concentration: %1 uM.")
                         .arg(QString::number(concs.value(toA01(seed.startWell)).toDouble(), 'f', 2));
        summaryLines << "";

        if (cmpDilutions.contains(standardKey)) {
          summaryLines << "  2. Serial Dilution Steps:";
          const auto &dilSteps = cmpDilutions[standardKey];
          int stepNum = 1;
          for (const auto &step : dilSteps) {
            summaryLines << QString("     - Step %1: Took %2 uL from well %3 and placed in well %4 (which contained %5 uL of DMSO).")
                             .arg(stepNum++)
                             .arg(QString::number(step.transferUL, 'f', 1))
                             .arg(toA01(step.srcWell))
                             .arg(toA01(step.dstWell))
                             .arg(QString::number(stdDmsoDilute, 'f', 1));
            summaryLines << QString("               Resulting concentration: %1 uM.")
                             .arg(QString::number(concs.value(toA01(step.dstWell)).toDouble(), 'f', 2));
          }
        }
        summaryLines << "";
      }
      ...
      // 2. Process Compounds
      QStringList cmpsSorted = cmpSeeds.keys();
      cmpsSorted.removeAll(standardKey);
      std::sort(cmpsSorted.begin(), cmpsSorted.end());

      for (const QString &cmpName : cmpsSorted) {
        const auto &seed = cmpSeeds.value(cmpName);
        const auto &plan = perPlan.value(cmpName, makeDefaultPlan());

        summaryLines << QString("Compound: %1").arg(cmpName);
        summaryLines << "--------------------------------------------------";
        summaryLines << QString("  1. Preparation of Starting Well (%1):").arg(toA01(seed.startWell));
        summaryLines << QString("     - Took %1 uL of %2 at %3 uM from matrix plate (barcode %4) at well %5.")
                         .arg(QString::number(seed.seedVolumeUL, 'f', 1))
                         .arg(cmpName)
                         .arg(QString::number(plan.stockConc, 'f', 1))
                         .arg(seed.matrixBarcode)
                         .arg(toA01(seed.matrixWell));
        summaryLines << QString("     - Placed in starting well %1 that contained %2 uL of DMSO.")
                         .arg(toA01(seed.startWell))
                         .arg(QString::number(plan.dmsoStart, 'f', 1));
        summaryLines << QString("     - Resulting starting concentration: %1 uM.")
                         .arg(QString::number(concs.value(toA01(seed.startWell)).toDouble(), 'f', 2));
        summaryLines << "";

        if (cmpDilutions.contains(cmpName)) {
          summaryLines << "  2. Serial Dilution Steps:";
          const auto &dilSteps = cmpDilutions[cmpName];
          int stepNum = 1;
          for (const auto &step : dilSteps) {
            summaryLines << QString("     - Step %1: Took %2 uL from well %3 and placed in well %4 (which contained %5 uL of DMSO).")
                             .arg(stepNum++)
                             ...
                             .arg(toA01(step.srcWell))
                             .arg(toA01(step.dstWell))
                             .arg(QString::number(plan.dmsoDilute, 'f', 1));
            summaryLines << QString("               Resulting concentration: %1 uM.")
                             .arg(QString::number(concs.value(toA01(step.dstWell)).toDouble(), 'f', 2));
          }
        }
    ...
    FileOut fsum;
    fsum.relativePath = QString("Audit/dilution_steps_summary.txt");
    fsum.lines = summaryLines;
    fsum.isAux = true;
    outs.push_back(std::move(fsum));
  }
