...
void TecanWindow::loadTestRequests(const QStringList &requestIDs) {
  ...
  connect(testRequestModel, &QAbstractItemModel::dataChanged, this, &TecanWindow::rebuildDaughterPlatesFromModels);
...
bool TecanWindow::checkFeasibility() {
  ...
  auto columnOf = [](const QAbstractItemModel *m, const QString &hdr) -> int {
    if (!m)
      return -1;
    QString target = hdr.trimmed().toLower().replace(" ", "_");
    for (int c = 0; c < m->columnCount(); ++c) {
      QString label = m->headerData(c, Qt::Horizontal).toString().trimmed().toLower().replace(" ", "_");
      if (label == target)
        return c;
    }
    // Substring fallback
  ...
  const int concCol = columnOf(cmpModel, "concentration");
  const int unitCol = columnOf(cmpModel, "concentration_unit");

  const int trCmpCol = columnOf(trModel, "compound_name");
  const int trStartCol = columnOf(trModel, "starting_concentration");
  const int trStartUnitCol = columnOf(trModel, "starting_concentration_unit");
  const int trTestCol = columnOf(trModel, "requested_tests");

  if (productCol < 0 || concCol < 0 || trCmpCol < 0 || trStartCol < 0 || trTestCol < 0) {
    qDebug() << "[checkFeasibility] Warning: column resolution failed. Feasibility check skipped."
             << "productCol:" << productCol
             << "concCol:" << concCol
             << "trCmpCol:" << trCmpCol
             << "trStartCol:" << trStartCol
             << "trTestCol:" << trTestCol;
    if (cmpModel) {
      QStringList cmpHeaders;
      for (int c = 0; c < cmpModel->columnCount(); ++c)
        cmpHeaders << cmpModel->headerData(c, Qt::Horizontal).toString();
      qDebug() << "  cmpModel headers:" << cmpHeaders;
    }
    if (trModel) {
      QStringList trHeaders;
      for (int c = 0; c < trModel->columnCount(); ++c)
        trHeaders << trModel->headerData(c, Qt::Horizontal).toString();
      qDebug() << "  trModel headers:" << trHeaders;
    }
    return true;
  }
  ...
  for (int r = 0; r < cmpModel->rowCount(); ++r) {
    const QString name = cmpModel->index(r, productCol).data().toString().trimmed();
    if (name.isEmpty()) continue;

    const double stockVal = cmpModel->index(r, concCol).data().toDouble();
    const QString stockUnit = unitCol >= 0 ? cmpModel->index(r, unitCol).data().toString().trimmed() : QString();
    double stockMicroM = GWLHelpers::convertToMicroM(stockVal, stockUnit);

    qDebug() << "[checkFeasibility] Compound:" << name << "Stock:" << stockVal << stockUnit << "=" << stockMicroM << "uM";

    // Find in trModel
    for (int trRow = 0; trRow < trModel->rowCount(); ++trRow) {
      const QString trName = trModel->index(trRow, trCmpCol).data().toString().trimmed();
      if (trName.compare(name, Qt::CaseInsensitive) == 0) {
        const double startVal = trModel->index(trRow, trStartCol).data().toDouble();
        const QString startUnit = trStartUnitCol >= 0 ? trModel->index(trRow, trStartUnitCol).data().toString().trimmed() : QString();
        double startMicroM = GWLHelpers::convertToMicroM(startVal, startUnit);

        const QString testId = trModel->index(trRow, trTestCol).data().toString().trimmed();
        const QJsonObject entry = catalogue.value(testId).toObject();
        if (entry.isEmpty()) {
          qDebug() << "  [checkFeasibility] Warning: Test ID" << testId << "not found in catalogue.";
          continue;
        }
        ...
        if (volFromDaughter <= 0.0) continue;

        double concDaughter_µM = 0.0;
        if (!isTarsal) {
          const double totalWellVol = entry.value("total_well_vol_µl").toDouble();
          if (totalWellVol <= 0.0) continue;
          concDaughter_µM = startMicroM * totalWellVol / volFromDaughter;
        } else {
          const double wellArea = entry.value("well_area_m2").toDouble();
          if (wellArea <= 0.0) continue;
          concDaughter_µM = (startMicroM * wellArea / (volFromDaughter * 1e-3)) * 1000.0;
        }

        qDebug() << "  [checkFeasibility] Match found. Start:" << startVal << startUnit << "=" << startMicroM << "uM | Test:" << testId << "| Required daughter conc:" << concDaughter_µM << "uM";

        if (concDaughter_µM > stockMicroM) {
          qDebug() << "  [checkFeasibility] INSUFFICIENT CONCENTRATION! Required:" << concDaughter_µM << "uM, Stock:" << stockMicroM << "uM";
          // Warning & Modify starting concentration!
          QMessageBox::StandardButton reply = QMessageBox::warning(
              this,
              tr("Feasibility Warning"),
              tr("The stock concentration of compound '%1' (%2 µM) is too low to reach the required starting concentration in the daughter plate (%3 µM) for test '%4'.\n\nWould you like to adjust the starting concentration in the test plate?")
                  .arg(name)
                  .arg(stockMicroM)
                  .arg(concDaughter_µM)
                  .arg(testId),
          ...
          if (reply == QMessageBox::Yes) {
            bool ok;
            double suggestedStartConc = stockMicroM * volFromDaughter / (isTarsal ? 1.0 : entry.value("total_well_vol_µl").toDouble()); // back-calculate back
            if (isTarsal) {
              suggestedStartConc = (stockMicroM / 1000.0) * (volFromDaughter * 1e-3) / entry.value("well_area_m2").toDouble();
            }
            double newConc = QInputDialog::getDouble(
                this,
                tr("Adjust Starting Concentration"),
                tr("Enter new starting concentration in test plate (%1):").arg(startUnit.isEmpty() ? "µM" : startUnit),
                startUnit.compare("mM", Qt::CaseInsensitive) == 0 ? suggestedStartConc / 1000.0 : suggestedStartConc,
                0.001, 1000000.0, 3, &ok
            );
            if (ok) {
              qDebug() << "  [checkFeasibility] User adjusting starting concentration of" << name << "to" << newConc;
              
              // Block signals to avoid immediate synchronous re-entry of rebuildDaughterPlatesFromModels
              const_cast<QAbstractItemModel*>(trModel)->blockSignals(true);
              const_cast<QAbstractItemModel*>(trModel)->setData(trModel->index(trRow, trStartCol), newConc);
              const_cast<QAbstractItemModel*>(trModel)->blockSignals(false);
              
              // Schedule asynchronous rebuild once current event loops finish, preventing nested checkFeasibility
              QMetaObject::invokeMethod(this, "rebuildDaughterPlatesFromModels", Qt::QueuedConnection);

              isChecking = false;
              return false; // stop current rebuild, async rebuild is scheduled
            }
          }
        }
      }
...
void TecanWindow::rebuildDaughterPlatesFromModels() {
  if (!checkFeasibility()) {
    return; // user is adjusting starting concentration
  }
  ...
  QMap<QString, int> compoundDilutions;
  if (trCmpCol >= 0 && nDilCol >= 0) {
      for (int i = 0; i < trModel->rowCount(); ++i) {
          QString cmpName = trModel->index(i, trCmpCol).data().toString().trimmed();
          int dil = trModel->index(i, nDilCol).data().toInt();
          if (!cmpName.isEmpty() && dil > 0) {
              compoundDilutions[cmpName] = dil;
          }
      }
  }

  const int defaultDilutionSteps =
      (nDilCol >= 0 && trModel->rowCount() > 0) ? trModel->index(0, nDilCol).data().toInt() : 3;
  const QString testType =
      (testTypeCol >= 0) ? trModel->index(0, testTypeCol).data().toString()
                         : QString();

  populateDaughterPlates(compoundDilutions, defaultDilutionSteps, compounds, testType);
}

void TecanWindow::populateDaughterPlates(const QMap<QString, int> &compoundDilutions, int defaultDilutionSteps, const QStringList &compoundList, const QString &testType) {
  ...
  QList<QMap<QString, QStringList>> plates = m_viewModel->getExperimentManager()->calculateInitialDaughterPlates(compoundDilutions, compoundList, testType, is384);

  // Compute maximum dilution for UI colors/drag defaults
  int maxDil = defaultDilutionSteps;
  for (int dil : compoundDilutions.values()) {
      if (dil > maxDil) maxDil = dil;
  }
  ...
  for (int i = 0; i < plates.size(); ++i) {
    ...
    plateWidget->setCompoundDilutions(compoundDilutions);
    ...
    plateWidget->populatePlate(plates[i], colours, maxDil);
    plateWidget->enableCompoundDragDrop(maxDil);
    daughterPlatesLayout->addWidget(plateWidget);
  }

  populateTestPlates(maxDil, testType, plates);
}
...
void TecanWindow::on_actionGenerate_GWL_triggered() {
  ...
  if (!checkFeasibility()) {
    qDebug() << "[TRACE] GWL generation aborted because feasibility check failed and user is adjusting concentration";
    return;
  }
...
static GwlGenResult runBackgroundGwlGeneration(
    ...
    GWLGenerator::Instrument instrument,
    QString outDir)
{
  GwlGenResult result;
  ...
  if (!generator.generate(experimentJson, outs, &err)) {
    ...
    result.errorString = QString("Generator error: %1").arg(err);
    return result;
  }
...
/* =======================================================================
 * 2) generateGWLFromJson() — uses new GWLGenerator and saveMany() with QtConcurrent
 * ======================================================================= */
void TecanWindow::generateGWLFromJson(const QJsonObject &experimentJson) {
  ...
  QProgressDialog *progress = new QProgressDialog(tr("Generating GWL and worklist files..."), QString(), 0, 0, this);
  progress->setWindowModality(Qt::WindowModal);
  progress->setRange(0, 0); // Indeterminate progress spinner
  progress->setWindowFlags(progress->windowFlags() & ~Qt::WindowCloseButtonHint);
  progress->show();

  // Create future watcher
  auto *watcher = new QFutureWatcher<GwlGenResult>(this);

  connect(watcher, &QFutureWatcher<GwlGenResult>::finished, this, [this, watcher, progress, experimentJson, outDir]() {
    GwlGenResult result = watcher->result();
    ...
    if (!result.success) {
      showError(this, tr("GWL Generation"), result.errorString);
      return;
    }
  ...
  QFuture<GwlGenResult> future = QtConcurrent::run(
      runBackgroundGwlGeneration,
      experimentJson,
      dilutionFactor,
      testId,
      stockConcMicroM,
...
void TecanWindow::populateTestPlates(int dilutionSteps, const QString &testType, const QList<QMap<QString, QStringList>> &daughterPlates) {
  ...
  QString stdName = "Standard";
  const QJsonObject stdObj = m_viewModel->getLastSavedExperimentJson()["standard"].toObject();
  if (!stdObj.isEmpty()) {
      stdName = stdObj.value("Samplealias").toString();
  }
  ExperimentManager::TestPlatesResult res = m_viewModel->getExperimentManager()->calculateTestPlates(dilutionSteps, testType, daughterPlates, m_viewModel->getQcPlatesJson(), qcType, stdName);
