    ...
    for (auto it = compoundWells.cbegin(); it != compoundWells.cend(); ++it)
    {
        ...
        for (int i = 0; i < wells.size(); ++i)
        {
            ...
            if (compound == "DMSO")
                ...
                int cmpDil = getDilution(compound);
                if (cmpDil <= 0) cmpDil = wells.size();
                if (cmpDil <= 0) cmpDil = dilutionSteps_;
                const qreal fade = 1.0 - (static_cast<qreal>(i) / qMax(1, cmpDil));
...
void DaughterPlateWidget::dropEvent(QDropEvent *e)
{
    ...
    int currentDilutionSteps = getDilution(compound);
    if (currentDilutionSteps <= 0) currentDilutionSteps = dilutionSteps_;
    ...
    for (int i = 0; i < currentDilutionSteps; ++i) {
    ...
    for (int i = 0; i < targetWells.size(); ++i)
    {
        ...
        const qreal fade = 1.0 - static_cast<qreal>(i) / qMax(1, currentDilutionSteps);
...
{
    int currentDilutionSteps = getDilution(cmpd);
    if (currentDilutionSteps <= 0) currentDilutionSteps = dilutionSteps_;
...
void DaughterPlateWidget::setCompoundDilutions(const QMap<QString, int> &dilutions)
{
    compoundDilutions_ = dilutions;
}

int DaughterPlateWidget::getDilution(const QString &compound) const
{
    QString key = compound.trimmed().toLower();
    for (auto it = compoundDilutions_.constBegin(); it != compoundDilutions_.constEnd(); ++it) {
