    ...
    , viewModel(std::make_unique<MainWindowViewModel>(this))
{
    ...
    // Create the "Register Compound" action and insert it into the toolbar
    actionRegisterCompound = new QAction(QIcon(":/icons/resources/icons/table.png"), "Register Selected Compound", this);
    actionRegisterCompound->setVisible(false);
    actionRegisterCompound->setEnabled(false);
    ui->toolBar->addAction(actionRegisterCompound);
    connect(actionRegisterCompound, &QAction::triggered, this, &MainWindow::onRegisterCompoundTriggered);

    // Create the "Create Solution from Bottle" action and insert it into the toolbar
    actionCreateSolutionFromBottle = new QAction(QIcon(":/icons/resources/icons/table.png"), "Create Solution from Bottle", this);
    actionCreateSolutionFromBottle->setVisible(false);
    actionCreateSolutionFromBottle->setEnabled(false);
    ...
    mBar->clear(); // Clear any loaded UI menus to avoid duplication

    // 1. Create Actions Menu
    QMenu* actionsMenu = mBar->addMenu(tr("&Actions"));
    actionsMenu->addAction(ui->actionAdd);
    actionsMenu->addAction(ui->refreshTableButton);
    actionsMenu->addAction(ui->actionexportCsvButton);
    actionsMenu->addAction(ui->actionTecan);
    ...
    // Helper lambda to dynamically clear and rebuild the theme menu items
    auto populateThemeMenu = [this, themeMenu]() {
        themeMenu->clear();
        ...
        for (const QString &themeName : themes) {
            ...
            connect(themeAction, &QAction::triggered, this, [themeName]() {
                ThemeManager::instance()->selectTheme(themeName);
            });
        }
    };
    ...
    // Connect theme manager's change signal to dynamically rebuild the menu bar
    connect(ThemeManager::instance(), &ThemeManager::themeChanged, this, populateThemeMenu);
    ...
    // Open Login Dialog
    if (!preAuthRole.isEmpty()) {
        setUserRole(preAuthRole);
        setupTreeView();
    } else {
...
void MainWindow::onTableSelected(const QItemSelection &selected, const QItemSelection &)
{
    ...
    if (tableName == "tracked_test_compounds") {
        actionRegisterCompound->setVisible(true);
        actionRegisterCompound->setEnabled(true);
    } else {
        actionRegisterCompound->setVisible(false);
        actionRegisterCompound->setEnabled(false);
    }
    ...
    if (tableName == "bottles") {
        actionCreateSolutionFromBottle->setVisible(true);
        actionCreateSolutionFromBottle->setEnabled(true);
    } else {
        actionCreateSolutionFromBottle->setVisible(false);
        actionCreateSolutionFromBottle->setEnabled(false);
    }
...
void MainWindow::onRegisterCompoundTriggered()
{
    QModelIndexList selectedRows = ui->dataTableView->selectionModel()->selectedRows();
    if (selectedRows.isEmpty()) {
        QMessageBox::warning(this, tr("No Selection"), tr("Please select a compound row to register."));
        return;
    }

    QModelIndex proxyIndex = selectedRows.first();

    // Map column values from the model
    int compoundNameCol = viewModel->getTableModel()->fieldIndex("compound_name");
    int trackedCompoundIdCol = viewModel->getTableModel()->fieldIndex("tracked_compound_id");
    int batchIdCol = viewModel->getTableModel()->fieldIndex("batch_id");
    int statusCol = viewModel->getTableModel()->fieldIndex("status");
    int additionalNotesCol = viewModel->getTableModel()->fieldIndex("additional_notes");
    int startingConcentrationCol = viewModel->getTableModel()->fieldIndex("starting_concentration");
    int stockConcentrationCol = viewModel->getTableModel()->fieldIndex("stock_concentration");
    int stockConcentrationUnitCol = viewModel->getTableModel()->fieldIndex("stock_concentration_unit");

    if (compoundNameCol == -1 || trackedCompoundIdCol == -1 || batchIdCol == -1) {
        QMessageBox::critical(this, tr("Database Error"), tr("Table schema mismatch: Required columns not found."));
        return;
    }

    QModelIndex nameIndex = viewModel->getProxyModel()->index(proxyIndex.row(), compoundNameCol);
    QModelIndex idIndex = viewModel->getProxyModel()->index(proxyIndex.row(), trackedCompoundIdCol);
    QModelIndex batchIndex = viewModel->getProxyModel()->index(proxyIndex.row(), batchIdCol);
    QModelIndex statusIndex = statusCol != -1 ? viewModel->getProxyModel()->index(proxyIndex.row(), statusCol) : QModelIndex();
    QModelIndex notesIndex = additionalNotesCol != -1 ? viewModel->getProxyModel()->index(proxyIndex.row(), additionalNotesCol) : QModelIndex();
    QModelIndex startConcIndex = startingConcentrationCol != -1 ? viewModel->getProxyModel()->index(proxyIndex.row(), startingConcentrationCol) : QModelIndex();
    QModelIndex stockConcIndex = stockConcentrationCol != -1 ? viewModel->getProxyModel()->index(proxyIndex.row(), stockConcentrationCol) : QModelIndex();
    QModelIndex stockConcUnitIndex = stockConcentrationUnitCol != -1 ? viewModel->getProxyModel()->index(proxyIndex.row(), stockConcentrationUnitCol) : QModelIndex();

    QString status = statusIndex.isValid() ? statusIndex.data().toString() : "";
    if (status == "sent" || status == "done") {
        auto ans = QMessageBox::question(this, tr("Already Registered"),
            tr("This compound has already been registered. Do you want to register it again?"),
            QMessageBox::Yes | QMessageBox::No);
    ...
    // Lookup project code from the batch
    QString batchIdStr = batchIndex.data().toString();
    QString projectCode;
    QSqlQuery q;
    q.prepare("SELECT project_code FROM public.tracked_test_batches WHERE batch_id = :batch_id::uuid");
    q.bindValue(":batch_id", batchIdStr);
    if (q.exec() && q.next()) {
        projectCode = q.value(0).toString();
    }

    QVariantMap compoundData;
    compoundData["compound_name"] = nameIndex.data().toString();
    compoundData["tracked_compound_id"] = idIndex.data().toString();
    compoundData["batch_id"] = batchIdStr;
    compoundData["project_code"] = projectCode;
    compoundData["additional_notes"] = notesIndex.isValid() ? notesIndex.data().toString() : "";
    compoundData["starting_concentration"] = startConcIndex.isValid() ? startConcIndex.data().toDouble() : 10.0;
    compoundData["stock_concentration"] = stockConcIndex.isValid() ? stockConcIndex.data().toDouble() : 0.0;
    compoundData["stock_concentration_unit"] = stockConcUnitIndex.isValid() ? stockConcUnitIndex.data().toString() : "";

    RegisterCompoundDialog dlg(compoundData, this);
    if (dlg.exec() == QDialog::Accepted) {
        refreshTableView();
    }
}

void MainWindow::onCreateSolutionFromBottleTriggered()
{
    QModelIndexList selectedRows = ui->dataTableView->selectionModel()->selectedRows();
    if (selectedRows.isEmpty()) {
        QMessageBox::warning(this, tr("No Selection"), tr("Please select a bottle row to dissolve."));
        return;
    }
    ...
    // Map column values from the model
    int bottleIdCol = viewModel->getTableModel()->fieldIndex("invenesis_bottle_id");
    int productNameCol = viewModel->getTableModel()->fieldIndex("product_name");
    int mwCol = viewModel->getTableModel()->fieldIndex("molecular_weight");
    int amountCol = viewModel->getTableModel()->fieldIndex("received_amount");
    int unitCol = viewModel->getTableModel()->fieldIndex("amount_unit");
    int purityCol = viewModel->getTableModel()->fieldIndex("purity");
    int projectCodeCol = viewModel->getTableModel()->fieldIndex("project_code");

    if (bottleIdCol == -1 || productNameCol == -1) {
        QMessageBox::critical(this, tr("Database Error"), tr("Table schema mismatch: Required columns not found."));
        return;
    }

    QModelIndex idIndex = viewModel->getProxyModel()->index(proxyIndex.row(), bottleIdCol);
    QModelIndex nameIndex = viewModel->getProxyModel()->index(proxyIndex.row(), productNameCol);
    QModelIndex mwIndex = mwCol != -1 ? viewModel->getProxyModel()->index(proxyIndex.row(), mwCol) : QModelIndex();
    QModelIndex amountIndex = amountCol != -1 ? viewModel->getProxyModel()->index(proxyIndex.row(), amountCol) : QModelIndex();
    QModelIndex unitIndex = unitCol != -1 ? viewModel->getProxyModel()->index(proxyIndex.row(), unitCol) : QModelIndex();
    QModelIndex purityIndex = purityCol != -1 ? viewModel->getProxyModel()->index(proxyIndex.row(), purityCol) : QModelIndex();
    QModelIndex projectCodeIndex = projectCodeCol != -1 ? viewModel->getProxyModel()->index(proxyIndex.row(), projectCodeCol) : QModelIndex();

    QVariantMap bottleData;
    bottleData["invenesis_bottle_id"] = idIndex.data().toString();
    bottleData["product_name"] = nameIndex.data().toString();
    bottleData["molecular_weight"] = mwIndex.isValid() ? mwIndex.data().toDouble() : 0.0;
    bottleData["received_amount"] = amountIndex.isValid() ? amountIndex.data().toDouble() : 0.0;
    bottleData["amount_unit"] = unitIndex.isValid() ? unitIndex.data().toString() : "mg";
    bottleData["purity"] = purityIndex.isValid() ? purityIndex.data().toDouble() : 0.0;
