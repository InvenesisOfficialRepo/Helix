...
bool DilutionEngine::loadVolumePlan(const QJsonObject &catalogue,
  ...
  // --- Step 1.5: cap required concentration to stock concentration if it exceeds it ---
  bool wasCapped = false;
  if (concDaughter_µM > stockConcMicroM) {
    qWarning() << "[WARN][DilutionEngine] Required daughter concentration" << concDaughter_µM
               << "uM exceeds stock concentration" << stockConcMicroM
               << "uM. Capping to stock concentration.";
    concDaughter_µM = stockConcMicroM;
    wasCapped = true;
  }
  ...
  double achievedFinalConc = startingConcInTestMicroM;
  if (wasCapped) {
    if (!isTarsal) {
      const double totalWellVol = entry.value("total_well_vol_\u00b5l").toDouble();
      achievedFinalConc = concDaughter_µM * volFromDaughter / totalWellVol;
    } else {
      const double wellArea = entry.value("well_area_m2").toDouble();
      achievedFinalConc = (concDaughter_µM / 1000.0) * (volFromDaughter * 1e-3) / wellArea;
    }
  }
  ...
  vp.finalConc = achievedFinalConc;
