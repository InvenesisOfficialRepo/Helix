...
QJsonObject buildExperimentJson(
    ...
    if (dilCol >= 0) {
        for (int r = 0; r < trModel->rowCount(); ++r) {
            int dVal = trModel->index(r, dilCol).data().toInt();
            if (dVal > dilutionSteps) {
                dilutionSteps = dVal;
            }
        }
    }
...
void addConcentrationsToExperimentJson(QJsonObject &root, const QJsonObject& qcPlatesJson) {
    ...
    if (!trArr.isEmpty()) {
        ...
        startingConcInTestMicroM = GWLHelpers::convertToMicroM(
            tr0.value("starting_concentration").toVariant().toDouble(),
    ...
    QMap<QString, CmpPlanKey> cmpBestPlan;
    for (const QJsonValue &rv : trArr) {
        QJsonObject ro = rv.toObject();
        QString cn = ro.value("compound_name").toString().trimmed().toLower();
        QString tid = ro.value("requested_tests").toString().trimmed();
        double sc = GWLHelpers::convertToMicroM(
            ro.value("starting_concentration").toVariant().toDouble(),
            ro.value("starting_concentration_unit").toString()
        );
        bool ok = false;
        double df = ro.value("dilution_steps").toVariant().toDouble(&ok);
        if (!ok || df <= 0.0) df = dilutionFactor;

        if (cn.isEmpty() || tid.isEmpty())
            continue;
        if (!cmpBestPlan.contains(cn) || sc > cmpBestPlan[cn].startConcMicroM) {
            cmpBestPlan[cn] = {tid, sc, df};
        }
    }
    ...
    for (const QJsonValue &v : cmpArr) {
        ...
        double sc = GWLHelpers::convertToMicroM(
            c.value("concentration").toVariant().toDouble(),
            c.value("concentration_unit").toString()
        );
        cmpStockMap[c.value("product_name").toString().trimmed().toLower()] = sc;
    }

    double stdStockConc = 0.0;
    QJsonObject stdObj = root.value("standard").toObject();
    if (!stdObj.isEmpty()) {
        stdStockConc = GWLHelpers::convertToMicroM(
            stdObj.value("Concentration").toVariant().toDouble(),
            stdObj.value("ConcentrationUnit").toString()
        );
    }
        ...
        for (const QJsonValue &v : plateArray) {
            ...
            for (auto it = cmpToWells.constBegin(); it != cmpToWells.constEnd(); ++it) {
                ...
                const bool isStandard = baseCmp.startsWith("Standard", Qt::CaseInsensitive) ||
                                        rawCmp.contains("Standard", Qt::CaseInsensitive);

                double dfForChain = dilutionFactor;
                QString cmpTestId = testId;
                double cmpStartConcMicroM = startingConcInTestMicroM;

                QString lowerCmp = baseCmp.trimmed().toLower();
                if (!isStandard) {
                    if (cmpBestPlan.contains(lowerCmp)) {
                        cmpTestId = cmpBestPlan[lowerCmp].testId;
                        cmpStartConcMicroM = cmpBestPlan[lowerCmp].startConcMicroM;
                        dfForChain = cmpBestPlan[lowerCmp].dilutionFactor;
                    }
                }
                ...
                if (type == "qc") {
                    ...
                    QString verr;
                    if (engine.loadVolumePlan(catalogue, cmpTestId, stock, topForBackCalc, &vp, &verr)) {
                        startConc = vp.concMother;
                        qDebug() << "[DEBUG] loadVolumePlan succeeded. startConc (concMother) =" << startConc;
                    } else {
                        qDebug() << "[DEBUG] loadVolumePlan failed in serializer for" << baseCmp << ":" << verr;
                        qDebug() << "[DEBUG] stock =" << stock << "topForBackCalc =" << topForBackCalc << "testId =" << cmpTestId;
                    }
                }
