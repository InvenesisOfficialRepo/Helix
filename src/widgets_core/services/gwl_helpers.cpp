...
double convertToMicroM(double value, const QString &unit) {
  QString u = unit.trimmed().toLower();
  if (u == "um" || u == "µm" || u == QString::fromUtf8("µm")) {
    return value;
  } else if (u == "mm") {
    return value * 1000.0;
  } else if (u == "nm") {
    ...
    // PPM is treated as numerically equivalent to uM/micromolar for ratio calculations
    return value;
  }
...
QList<DaughterPlateEntry> fillMissingWells(
    const QList<DaughterPlateEntry> &existingRows,
    const QString &barcode,
    bool is384,
    const QString &projectCode,
    const QString &dilutionSteps
) {
    QMap<QString, DaughterPlateEntry> wellMap;
    for (const auto &r : existingRows) {
        wellMap.insert(r.wellA01, r);
    }

    QList<DaughterPlateEntry> allRows;
    QStringList rowLetters = is384 ? QStringList{"A","B","C","D","E","F","G","H","I","J","K","L","M","N","O","P"}
                                   : QStringList{"A","B","C","D","E","F","G","H"};
    int maxCols = is384 ? 24 : 12;

    for (const QString &r : rowLetters) {
        for (int c = 1; c <= maxCols; ++c) {
            const QString well = r + QString::number(c).rightJustified(2, '0');
            if (wellMap.contains(well)) {
                allRows.push_back(wellMap.value(well));
            } else {
                DaughterPlateEntry emptyEntry;
                emptyEntry.containerBarcode = barcode;
                emptyEntry.sampleAlias = "NA";
                emptyEntry.wellA01 = well;
                emptyEntry.volumeUL = 0.0;
                emptyEntry.volumeUnit = "ul";
                emptyEntry.conc = 0.0;
                emptyEntry.concUnit = "uM";
                emptyEntry.u1 = QString("%1_%2").arg(barcode, well);
                emptyEntry.u2 = "NA";
                emptyEntry.u3 = "0.0000";
                emptyEntry.u4 = dilutionSteps;
                emptyEntry.u5 = projectCode;
                allRows.push_back(emptyEntry);
            }
        }
    }
    return allRows;
